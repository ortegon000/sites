...
- `resources/views/pages/contacts/⚡{index,show}.blade.php` — el directorio de personas y la vista "todo lo de Juan": sus empresas con cargo, número de proyectos y dominios. Viven **dentro de Clientes**, no como sección aparte del CRM: las rutas cuelgan de `clientes/contactos` y se cambia entre listas con pestañas (`resources/views/partials/clients-nav.blade.php`), en vez de ocupar un renglón propio en la barra lateral. `clientes/contactos` se registra **antes** que `clientes/{client}`, o el comodín se la traga; hay una prueba que lo fija. Las pestañas no aparecen en `/prospectos`, que comparte componente con la lista de empresas. El directorio **no crea personas**: solo las lista y edita. Registrarlas es atribución del detalle de cada empresa, porque un contacto sin empresa no le sirve a nadie y permitirlo desde el directorio llenaría la lista de gente suelta.
